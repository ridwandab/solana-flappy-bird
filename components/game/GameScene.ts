...
export class GameScene extends Phaser.Scene {
  ...
  private createScrollingBackground() {
    ...
    if (this.textures.exists('background_sprite')) {
      // Create two background tiles for seamless scrolling - optimized height
      ...
      // Create fallback background with night city theme colors - optimized height
    ...
    // Set camera bounds to optimized height (1000px total)
  ...
  private createStartScreen() {
    ...
    // Create bird for start screen (static, no physics) - centered for optimized height
  ...
  private initializeGame() {
    ...
    if (this.bird) {
      ...
      console.log('Bird converted from static to physics sprite')
        ...
        this.bird = this.physics.add.sprite(200, 500, 'bird_default')
        this.bird.setScale(0.15) // Slightly smaller for better proportion with pipe gap
        this.bird.setVisible(true)
        this.bird.setAlpha(1)
  ...
  private spawnPipe() {
    ...
    const gameHeight = 1000 // Game height - optimized for no scroll
    
    // Use random gap position for more challenging gameplay - optimized viewport
    const minGapPosition = 200 // Minimum gap position from top (optimized viewport)
    const maxGapPosition = 700 // Maximum gap position from top (optimized viewport)
      ...
      callback: () => {
        if (this.bird && this.bird.body) {
          ...
          this.bird.angle = Math.min(Math.max(velocity * 0.15, -90), 90)
            ...
            if (this.bird.y >= 980) { // Ground level - bottom ground for 1000px height
              this.bird.y = 980
            const body = this.bird.body as Phaser.Physics.Arcade.Body
  ...
  private createGameOverPopup() {
    // Create popup background (smaller and centered for 1000px height)
