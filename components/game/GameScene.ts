...
export class GameScene extends Phaser.Scene {
  ...
  private createScrollingBackground() {
    ...
    if (this.textures.exists('background_sprite')) {
      // Create two background tiles for seamless scrolling - top same, bottom raised more
      this.background1 = this.add.tileSprite(400, 465, 800, 930, 'background_sprite')
      this.background2 = this.add.tileSprite(1200, 465, 800, 930, 'background_sprite')
      
      console.log('Scrolling background created with Background5.png')
    } else {
      // Create fallback background with night city theme colors - top same, bottom raised more
      this.background1 = this.add.rectangle(400, 465, 800, 930, 0x1a1a2e)
      this.background2 = this.add.rectangle(1200, 465, 800, 930, 0x1a1a2e)
    ...
    // Set camera bounds to top same, bottom raised more (930px total)
    this.cameras.main.setBounds(0, 0, 800, 930)
  }
  ...
  private createStartScreen() {
    ...
    // Create bird for start screen (static, no physics) - centered for top same, bottom raised more
    this.bird = this.add.sprite(400, 465, 'bird_default')
    ...
    const tapToStartText = this.add.text(400, 565, 'Tap anywhere to start', {
      fontSize: '24px',
      color: '#ffffff',
      fontFamily: 'Arial',
      fontStyle: 'bold',
  ...
  private initializeGame() {
    ...
    if (this.bird) {
      ...
      console.log('Bird converted from static to physics sprite')
        ...
        this.bird = this.physics.add.sprite(200, 465, 'bird_default')
        this.bird.setScale(0.15) // Slightly smaller for better proportion with pipe gap
        this.bird.setVisible(true)
        this.bird.setAlpha(1)
  ...
  update() {
    ...
    for (let i = 0; i < this.activePipes.length; i++) {
      ...
      // Only check collision if pipe is close to bird (within 100 pixels for better performance)
      if (Math.abs(pipeSet.topPipe.x - this.bird.x) < 100) {
        ...
        if (pipeSet.topPipeCollision) {
          // Fair visual margin - balanced collision sensitivity
          const visualMargin = 5 // Balanced margin for fair gameplay
        ...
        if (pipeSet.topPipeCollision && pipeSet.bottomPipeCollision) {
          ...
          const gapTolerance = 10 // Balanced tolerance for gap detection
  ...
  private spawnPipe() {
    ...
    const maxGapPosition = 680 // Maximum gap position from top (top same, bottom raised more)
    ...
    const difficultyVariation = Math.min(this.difficultyLevel * 19, 190) // Increase variation with difficulty
    const finalGapPosition = Phaser.Math.Between(
      Math.max(minGapPosition - difficultyVariation, 140), // Can go higher up
      Math.min(maxGapPosition + difficultyVariation, 780)  // Can go lower down
    )
  ...
  private createGameOverPopup() {
    // Create popup background (smaller and centered for 930px height)
    const popupBg = this.add.rectangle(400, 465, 320, 260, 0x2C2C2C, 0.3)
    popupBg.setStrokeStyle(3, 0x444444)
    popupBg.setOrigin(0.5)
    
    // Add subtle shadow effect
    const shadow = this.add.rectangle(403, 468, 320, 260, 0x000000, 0.1)
    shadow.setOrigin(0.5)
    shadow.setDepth(-1)
    
    // Game Over title
    const gameOverText = this.add.text(400, 385, 'GAME OVER', {
      fontSize: '32px',
      color: '#ff4444',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Score display
    const scoreText = this.add.text(400, 425, `Score: ${this.score}`, {
      fontSize: '20px',
      color: '#ffff00',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    const highScoreText = this.add.text(400, 455, `High Score: ${currentHighScore}`, {
      fontSize: '20px',
      color: '#00ff00',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Main Menu button
    const mainMenuBtn = this.add.rectangle(330, 510, 120, 40, 0x4a90e2)
    mainMenuBtn.setStrokeStyle(3, 0x000000)
    mainMenuBtn.setInteractive()
    
    const mainMenuText = this.add.text(330, 510, 'Main Menu', {
      fontSize: '20px',
      color: '#ffffff',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Restart button
    const restartBtn = this.add.rectangle(470, 510, 120, 40, 0x00ff00)
    restartBtn.setStrokeStyle(3, 0x000000)
    restartBtn.setInteractive()
    
    const restartText = this.add.text(470, 510, 'Restart', {
      fontSize: '20px',
      color: '#000000',
      fontFamily: 'Arial',
      fontStyle: 'bold'
