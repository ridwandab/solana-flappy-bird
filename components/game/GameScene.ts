...
export class GameScene extends Phaser.Scene {
  ...
  private createScrollingBackground() {
    ...
    if (this.textures.exists('background_sprite')) {
      // Create two background tiles for seamless scrolling - extended height with top area
      this.background1 = this.add.tileSprite(400, 600, 800, 1400, 'background_sprite')
      this.background2 = this.add.tileSprite(1200, 600, 800, 1400, 'background_sprite')
      
      console.log('Scrolling background created with Background5.png')
    } else {
      // Create fallback background with night city theme colors - extended height with top area
      this.background1 = this.add.rectangle(400, 600, 800, 1400, 0x1a1a2e)
      this.background2 = this.add.rectangle(1200, 600, 800, 1400, 0x1a1a2e)
    ...
    // Set camera bounds to extended height with top area (1400px total)
  ...
  private createStartScreen() {
    ...
    this.ground = this.add.rectangle(400, 1380, 800, 40, 0x8B4513)
    this.ground.setScrollFactor(0)
    this.ground.setVisible(false) // Hide ground visually but keep collision
    this.startScreenElements.push(this.ground)

    // Create bird for start screen (static, no physics) - centered for extended height
  ...
  private initializeGame() {
    ...
    // Score text
    this.scoreText = this.add.text(400, 100, '0', {
      fontSize: '48px',
      color: '#ffffff',
      stroke: '#000000',
      strokeThickness: 4,
    ...
    // Difficulty text
    this.difficultyText = this.add.text(400, 160, 'Level 0', {
      fontSize: '24px',
      color: '#ffff00',
      stroke: '#000000',
      strokeThickness: 3,
  ...
  private spawnPipe() {
    ...
    const gameHeight = 1400 // Game height - extended with top area
    
    // Use random gap position for more challenging gameplay - extended viewport
    const minGapPosition = 350 // Minimum gap position from top (extended viewport)
    const maxGapPosition = 1050 // Maximum gap position from top (extended viewport)
    ...
    const difficultyVariation = Math.min(this.difficultyLevel * 30, 300) // Increase variation with difficulty
    const finalGapPosition = Phaser.Math.Between(
      Math.max(minGapPosition - difficultyVariation, 300), // Can go higher up
      ...
      callback: () => {
        if (this.bird && this.bird.body) {
          ...
          this.bird.angle = Math.min(Math.max(velocity * 0.15, -90), 90)
            ...
            if (this.bird.y >= 1380) { // Ground level - bottom ground for 1400px height
              this.bird.y = 1380
            const body = this.bird.body as Phaser.Physics.Arcade.Body
  ...
  private createGameOverPopup() {
    ...
    const popupBg = this.add.rectangle(400, 700, 320, 260, 0x2C2C2C, 0.3)
    popupBg.setStrokeStyle(3, 0x444444)
    popupBg.setOrigin(0.5)
    
    // Add subtle shadow effect
    const shadow = this.add.rectangle(403, 703, 320, 260, 0x000000, 0.1)
    shadow.setOrigin(0.5)
    shadow.setDepth(-1)
    
    // Game Over title
    const gameOverText = this.add.text(400, 620, 'GAME OVER', {
      fontSize: '32px',
      color: '#ff4444',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Score display
    const scoreText = this.add.text(400, 660, `Score: ${this.score}`, {
      fontSize: '20px',
      color: '#ffff00',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    const highScoreText = this.add.text(400, 690, `High Score: ${currentHighScore}`, {
      fontSize: '20px',
      color: '#00ff00',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Main Menu button
    const mainMenuBtn = this.add.rectangle(330, 745, 120, 40, 0x4a90e2)
    mainMenuBtn.setStrokeStyle(3, 0x000000)
    mainMenuBtn.setInteractive()
    
    const mainMenuText = this.add.text(330, 745, 'Main Menu', {
      fontSize: '20px',
      color: '#ffffff',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Restart button
    const restartBtn = this.add.rectangle(470, 745, 120, 40, 0x00ff00)
    restartBtn.setStrokeStyle(3, 0x000000)
    restartBtn.setInteractive()
    
    const restartText = this.add.text(470, 745, 'Restart', {
      fontSize: '20px',
      color: '#000000',
      fontFamily: 'Arial',
      fontStyle: 'bold'
