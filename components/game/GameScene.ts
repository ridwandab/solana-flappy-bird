...
export class GameScene extends Phaser.Scene {
  ...
  private createStartScreen() {
    ...
    // Create invisible ground for collision detection only (adjusted for 900px height)
    ...
    const tapToStartText = this.add.text(400, 540, 'Tap anywhere to start', {
      fontSize: '24px',
      color: '#ffffff',
      fontFamily: 'Arial',
      fontStyle: 'bold',
  ...
  update() {
    ...
    if (this.bird && !this.isGameOver) {
      ...
      const groundTop = 880 // Ground top edge (ground is at y: 880, height: 40)
    ...
    this.time.addEvent({
      ...
      callback: () => {
        if (this.bird && this.bird.body) {
          ...
          if (this.bird.y >= 880) { // Ground level (adjusted for 900px height)
            this.bird.y = 880
            const body = this.bird.body as Phaser.Physics.Arcade.Body
