...
export class GameScene extends Phaser.Scene {
  ...
  private createScrollingBackground() {
    ...
    // Set camera bounds to extended height (1000px total)
    ...
    this.time.addEvent({
      ...
      callback: () => {
        if (this.bird && this.bird.body) {
          ...
          if (this.bird.y >= 800) { // Ground level - bottom ground
            this.bird.y = 800
            const body = this.bird.body as Phaser.Physics.Arcade.Body
  ...
  private createGameOverPopup() {
    // Create popup background (smaller and centered)
    const popupBg = this.add.rectangle(400, 400, 320, 260, 0x2C2C2C, 0.3)
    popupBg.setStrokeStyle(3, 0x444444)
    popupBg.setOrigin(0.5)
    
    // Add subtle shadow effect
    const shadow = this.add.rectangle(403, 403, 320, 260, 0x000000, 0.1)
    ...
    // Main Menu button
    const mainMenuBtn = this.add.rectangle(330, 445, 120, 40, 0x4a90e2)
    mainMenuBtn.setStrokeStyle(3, 0x000000)
    mainMenuBtn.setInteractive()
    
    const mainMenuText = this.add.text(330, 445, 'Main Menu', {
      fontSize: '20px',
      color: '#ffffff',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Restart button
    const restartBtn = this.add.rectangle(470, 445, 120, 40, 0x00ff00)
    restartBtn.setStrokeStyle(3, 0x000000)
    restartBtn.setInteractive()
    
    const restartText = this.add.text(470, 445, 'Restart', {
      fontSize: '20px',
      color: '#000000',
      fontFamily: 'Arial',
      fontStyle: 'bold'
